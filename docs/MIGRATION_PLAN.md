...
### Phase 4 — Break apart `app.js` — COMPLETE
...
- **`auth.js` extracted, 2026-09-16**: the whole OTP-only login/registration flow — `normalizePhoneNumber`/`applyAuthLockState`/`togglePasswordVisibility`, the `AuthFlow` state object and `COUNTRY_CODES` table, `initAuthGate`/`enterApp` (boot gate), step navigation (`showStep`/`showRequestStep`/`showRegisterStep`/`renderWizardDots`), channel + country-code selection (`setLoginMethod`/`buildCountryList`/`toggleCountryList`/`pickCountry`/`onPhoneInput`), OTP send/resend/verify with the resend countdown (`requestOtp`/`openOtpStep`/`maskTarget`/`startResendTimer`/`editOtpTarget`/`resendOtp`/`clearOtpBoxes`/`readOtpCode`/`onOtpInput`/`onOtpKeydown`/`onOtpPaste`/`verifyOtpCode`), Google OAuth (`signInWithGoogle`), registration/onboarding (`submitRegistration`/`onRegGstinInput`/`pickIndustry`/`finishOnboarding`/`hydrateTenantFromShop`), and sign-out/lock (`setAuthBusy`/`fullSignOut`/`lockPOS`) — 34 functions plus the `AuthFlow`/`COUNTRY_CODES` literals, 560 lines. Deliberately excludes `applyRoleSecurity` (role-based UI visibility; sits just above the auth section header but is pinned to `app.js` by `tests/permissions-parity.js`, which reads it directly out of `app.js`'s source text to eval and test against its typed `src/core/permissions/` port — moving it would break that harness) and `hydrateCloudData` (the cloud data-pull + invoice-counter-repair routine physically interleaved inside the auth block, between `hydrateTenantFromShop` and `setAuthBusy`; it's called from `initAuthGate()` but is *also* called from `refreshFromCloud()` in the offline-sync block, outside the auth region, making it a cross-cutting data-integrity function rather than auth-flow UI — left in `app.js`, called by name from `auth.js` exactly as every other extracted engine calls into `app.js` globals). `applyAuthLockState`/`lockPOS` are called from outside this file (a `window.applyAuthLockState` exposure and index.html's "Lock POS" button `onclick="lockPOS()"`) — both keep working unchanged via classic shared global scope. Checked git history between `1f56ed1` and HEAD for all 34 functions: none were touched by the F1–F5 security fixes or the Phase 6 IndexedDB cutover (only `hydrateCloudData`, which stayed in `app.js`, was touched — by Phase 6), so all 34 are byte-identical to their pre-extraction `app.js` bodies with no exclusions needed — verified by `tests/extraction-parity.js` (34/34 green, 104/104 overall). Full suite green (13 files); `tsc --noEmit` clean; real-browser boot test (Chromium via Playwright) confirmed `AuthFlow`/`initAuthGate`/`APP_STATE` are defined on first load and survive a full page reload with no new console errors (the pre-existing Supabase-CDN-blocked-by-sandbox-network error is unchanged from prior extractions' notes).
- `app.js` now 2,731 lines (from 5,442 originally; 3,215 before this extraction).
- **`pos.js` extracted, 2026-09-16 — the final piece**: the counter-facing POS/checkout flow — the UPI QR renderer (`generateDynamicUpiQR`), the camera/barcode scanner engine (`openCameraScanner`/`beepScanFeedback`/`toggleScanContinuous`/`submitManualScan`/`closeCameraScanner`/`handleScannedCode`), the item-attribute modal and cart (`openItemModal`/`closeModal`/`r2`/`commitModalItem`/`renderCart`*/`removeCart`/`setTender`/`setStep`/`jumpToStep`), out-of-stock alternatives (`showAlternativesFor`/`selectAlternative`), the post-checkout invoice-action popup (`openInvoiceActionPopup`/`closeInvoiceActionPopup`/`getIapSale`/`iapDownload`/`iapPrint`/`iapOpenEdit`/`iapSaveEdit`/`iapOpenCancel`), checkout itself (`reserveInvoiceNumber`/`checkoutBill`/`syncInvoiceToCloud`*), and invoice printing (`INDUSTRY_INVOICE_PROFILES`/`applyIndustryInvoiceTheme`/`printA4Invoice`/`renderHsnTaxBreakup`) — 29 functions plus the `INDUSTRY_INVOICE_PROFILES` const, 1,070 lines. `*renderCart` (F2, `1563ff5`) and `*syncInvoiceToCloud` (F4 `88a5f5d` + F5 `b90c609`) are excluded from the byte-identical parity check specifically — both carry genuine, reviewed post-`1f56ed1` fixes, same precedent as `submitReturn`/`recordPurchaseBill` in the returns/purchases extractions; every other function in this file is byte-identical to its pre-extraction `app.js` body. `r2()` and `setStep()`/`jumpToStep()` move here as the POS-owned versions of helpers that `reports.js` and `customers.js` also call by name across the shared classic-script global scope — unaffected by which file declares them. **`SyncEngine`, `AuthFlow`, and the checkout/invoice RPC call sequence (`reserveInvoiceNumber` → build invoice → push to `APP_STATE.sales` → decrement stock → update customer ledger → `syncInvoiceToCloud` (fire-and-forget) → print → reset cart, with `SB.saveSale`'s single `create_invoice_atomic` RPC doing the insert/stock-decrement/ledger-upsert atomically) were NOT touched** — `checkoutBill`/`reserveInvoiceNumber` moved byte-for-byte, `SyncEngine` stays in `app.js` untouched, `AuthFlow` (in `auth.js`) was never referenced. Nothing was left behind: every candidate in scope moved cleanly. Verified by `tests/extraction-parity.js` (29/29 green, 134/134 overall), full suite green (13 files, 0 failures), `tsc --noEmit` clean, and a real-browser boot test (Chromium via Playwright) confirming `checkoutBill`/`renderCart`/`APP_STATE`/`SyncEngine`/`openItemModal` are all defined post-load, `APP_STATE.inventory` survives a full page reload, and the console-error set is byte-identical to the pre-extraction baseline (only the pre-existing sandbox-network-blocked Supabase CDN errors, unchanged).
- `app.js` now 1,743 lines (from 5,442 originally; 2,731 before this extraction). `pos.js` is 1,070 lines. **Phase 4 complete — all `app.js` feature extractions done.**
- **Explicit stop condition, held for the whole of Phase 4**: any extraction that would require changing how `AuthFlow`, `SyncEngine`, or the checkout/invoice RPC call sequence work was never a Phase 4 change — that would be Phase 5 territory. Confirmed true across every extraction in this phase, including this final POS/checkout one: `SyncEngine` and `AuthFlow` were never moved or edited, and `checkoutBill`/`syncInvoiceToCloud`'s RPC sequencing is unchanged from before Phase 4 began.
