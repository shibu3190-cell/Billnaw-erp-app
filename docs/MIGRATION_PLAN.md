...
- **`inventory.js` extracted, 2026-09-16**: `renderInventoryTable`/`openEditStockModal`/`closeEditStockModal`/`saveEditedStock`/`deleteInventoryItemPrompt` (stock-master table) and `resetCatalogPaging`/`loadMoreCatalog`/`renderPagerFooter`/`renderCatalog`/`openNewProductModal`/`closeNewProdModal`/`saveNewProduct` (catalogue grid + new product), 12 functions, byte-identical to their pre-extraction bodies per `tests/extraction-parity.js`. Deliberately excludes `openItemModal`/`commitModalItem`/`showAlternativesFor`/`selectAlternative` — those look inventory-adjacent but are actually the POS add-to-cart flow, staying with checkout. `app.js` now 4,168 lines (from 5,442 originally).
- **Not started**: `app.js` still contains auth (`AuthFlow`), POS/checkout, returns, and reports. Order per `docs/ARCHITECTURE_TARGET.md` §8 complexity table: returns → reports next (medium complexity), auth and POS/checkout last (highest coupling, highest financial stakes).
