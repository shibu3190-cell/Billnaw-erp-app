...
### Phase 6 — Local database — FIRST INCREMENT DONE (2026-09-15), cutover NOT STARTED
...
- **What's done**: `src/core/database/index.ts` — a typed IndexedDB layer (chosen over SQLite-via-WASM as the lower-complexity option, matching what the app already runs natively in every browser with no new binary/WASM dependency). One object store per existing `bn_*` localStorage key, keyed to match the app's own existing identity scheme exactly: `inventory` by `id`, `customers` by `phone` (the real `(shop_id, phone)` unique constraint), `sales`/`returns`/`purchases` by `idempotency_key` (the exact key `SyncEngine.generateIdempotencyKey()` already produces — no new identity scheme invented), `syncQueue` autoIncrement, `meta` for scalars (`invCounter`, `tenantProfile`, etc.). `tests/local-database.js` (10/10 passing, using `fake-indexeddb` for a real IndexedDB implementation in Node) verifies correctness, including a 5,000-record scale test proving single-record reads/writes don't require touching the rest of the dataset — the actual defect in the current `localStorage` pattern (`persistState()` rewrites the entire inventory/customers/sales/returns/purchases arrays as JSON strings on every single state change), not just its ~5-10MB size cap.
- **What's explicitly NOT done**: `app.js`/`index.html` are completely untouched — verified via `git status` before committing. Nothing reads from or writes to IndexedDB yet; `localStorage` remains the only thing the deployed app actually uses. This was a deliberate stop, not an oversight — the actual cutover (migrating existing users' `localStorage` data into IndexedDB, rewriting `persistState()`/`loadState()`/`SyncEngine` to use the new layer, and validating nothing regresses for a live financial app) is a separate, larger, higher-risk step that still requires its own explicit go-ahead per this phase's original gate. Do not read "Phase 6 done" as "cutover done" — only the foundation is built and proven.
