...
- `app.js` now 3,921 lines (from 5,442 originally). Full suite: 13 files, 306 assertions, green; `tsc --noEmit` clean; real-browser boot test re-verified after each extraction.
- **Not started**: `app.js` still contains auth (`AuthFlow`), POS/checkout, and reports. Order per `docs/ARCHITECTURE_TARGET.md` §8 complexity table: reports next (medium complexity), auth and POS/checkout last (highest coupling, highest financial stakes).
