...
- **No automated test touches Supabase, RLS, or the edge functions.** All server-side logic — the atomic RPCs, role-based redaction, Gemini reconciliation — is untested by anything in the repo; only covered by the manual RUNBOOK test plan. This is the most consequential test-coverage gap for a financial system.
...
6. **Add automated tests for the atomic RPCs and RLS policies** (§8 gap) before or alongside any schema-touching work in later phases — this is the biggest blind spot for a financial system and should not wait for Phase 8+.
