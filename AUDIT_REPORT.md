...
- ~~No automated test touches Supabase, RLS, or the edge functions.~~ **Resolved 2026-09-17**: `tests/rpc-rls-tests.js` (`npm run test:rpc`) now runs 33 automated checks against a real PostgreSQL instance with every migration applied and RLS genuinely enforced (connected as a non-superuser, non-owner role — the only way RLS isn't silently bypassed), covering tenant isolation, shop revocation, role-based financial redaction, and the atomic invoice/purchase/return RPCs' idempotency and rollback behavior. This suite surfaced a real HIGH-severity finding the read-only audit missed — no INSERT policy existed anywhere for `shops`/`profiles`, confirmed broken in production by querying the live project's `pg_policies` directly — now fixed via migration `0010_shop_signup_atomic.sql`. See SECURITY_REPORT.md F0 for the full history. Gemini reconciliation (the edge function) remains untested by anything in the repo — that gap is still open.
...
6. ~~Add automated tests for the atomic RPCs and RLS policies~~ **Done 2026-09-17** — see `tests/rpc-rls-tests.js`. Surfaced and led to fixing a real HIGH-severity gap (no INSERT policy on `shops`/`profiles`, confirmed broken in production — SECURITY_REPORT.md F0, now resolved by migration `0010_shop_signup_atomic.sql`, still needs applying to the live project).
