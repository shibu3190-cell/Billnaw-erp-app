/* ==========================================================================
   BILLNAW — REPORTS & DASHBOARD DRILL-THROUGH ENGINE (parts 1 + 2)
   Extracted verbatim from app.js (Phase 4 of the modernization migration —
   structural relocation only, no logic changed). Part 1: the Reports hub
   entry point (openReport) and dashboard KPI-card drill-through. Part 2:
   dashboard rendering (renderDashboard/renderActivityFeed/
   renderMobileInvoiceCards/filterReportsCategory), receivables ageing and
   KPI/donut-chart rendering, the orders/sales trend chart, and the current-
   report CSV export + print.

   The reports/dashboard code in app.js was NOT one contiguous block — it's
   interleaved with the Subscription & Billing panel, the generic
   switchView() navigation utility (used across every feature, not report-
   specific, so it stays in app.js), search, sidebar, GST/printer settings,
   and the Khata CSV export (exportData(), customer-specific, not a report
   — also stays in app.js). This file is therefore assembled from six
   separate ranges cut out of app.js across two commits, each individually
   contiguous and verified, rather than one artificial single cut.
...
window.renderMobileInvoiceCards = renderMobileInvoiceCards;
window.computeReceivablesAgeing = computeReceivablesAgeing;
window.renderKpiDeltas = renderKpiDeltas;
