/* ==========================================================================
   BILLNAW — GST CONFIGURATION & TAX ENGINE (typed extraction)

   This is a typed, ES-module port of the runtime gstConfig.js that
   ...
   Until that frontend exists (Phase 4+ of MIGRATION_PLAN.md), gstConfig.js
   stays the single deployed source of truth — nothing in index.html
   changed. The two files are kept in lockstep by tests/gst-parity.js,
   which runs the same input matrix through both and fails the build the
   moment they disagree. Treat any parity-test failure as a real bug, not
   a test to "fix" by relaxing it.
   ========================================================================== */
...
// Adding a slab here does NOT retroactively change any saved invoice —
// each line captures the rate it was billed at (see computeLine below).
const DEFAULT_GST_SLABS: number[] = [0, 0.25, 3, 5, 12, 18, 28];

export interface GstConfigShape {
  slabs: number[];
  cessEnabled: boolean;
  updatedAt: string | null;
}

export interface SetSlabsResult {
  slabs?: number[];
  error?: string;
}
...
export const GstConfig = {
  ...
  save(cfg: GstConfigShape): void {
    cfg.updatedAt = new Date().toISOString();
    localStorage.setItem(this.storageKey, JSON.stringify(cfg));
  },

  getSlabs(): number[] {
    return this.load().slabs;
  },

  setSlabs(slabArray: Array<number | string>): SetSlabsResult {
  ...
  // DOM-coupled — kept for interface parity with the runtime module, but
  // intentionally a no-op here since this module has no window/document
  // dependency the way the legacy script does. Real usage moves to a React
  // effect/hook in the Phase 8 UI migration, not this call.
  refreshAllRateSelects(): void {
    // no-op in the typed module; see class doc comment above.
  },
};
...
export interface ComputeLineResult {
  ...
  gstAmount: number;
  totalAmount: number;
  gstRateAtBilling: number;
  gstModeAtBilling: 'inclusive' | 'exclusive';
}
...
export const TaxEngine = {
  ...
  resolvePlaceOfSupply({ customerGstin, customerStateCode, shopStateCode }: PlaceOfSupplyInput): string {
  ...
  isInterstate({ customerGstin, customerStateCode, shopStateCode }: PlaceOfSupplyInput): boolean {
  ...
  computeLine({ price, qty, gstRate, includeGst = false }: ComputeLineInput): ComputeLineResult {
  ...
  computeInvoiceTotals(lines: InvoiceLineForTotals[]): InvoiceTotals {
  ...
  groupByHsn(items: HsnLineItem[], interstate: boolean): HsnGroup[] {
    const groups: Record<string, HsnGroup> = {};
    items.forEach((it) => {
      const rate = it.gstRateAtBilling !== undefined ? it.gstRateAtBilling : (it.gst as number);
      const key = `${it.hsn}|${rate}`;
      if (!groups[key]) {
        groups[key] = { hsn: it.hsn, gstRate: rate, taxable: 0, gstAmt: 0, cgst: 0, sgst: 0, igst: 0 };
      }
