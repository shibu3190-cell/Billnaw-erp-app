/* ==========================================================================
   BILLNAW — SUPABASE INTEGRATION LAYER (typed extraction, Phase 5)

   This is a typed, ES-module port of the runtime supabaseClient.js that
   index.html still loads as a classic <script> today. It exists ahead of
   the React/Vite frontend that will eventually import it directly.

   Until that frontend exists, supabaseClient.js's global `SB` namespace
   stays the single deployed source of truth — nothing in index.html
   changed. Every method body below is byte-identical to its counterpart
   in supabaseClient.js (same local variable names `_sb`/`SUPABASE_URL`
   deliberately kept so the two can be diffed directly), verified by
   tests/supabase-parity.js. Treat any parity-test failure as a real
   contract change to review, not a test to "fix" by relaxing it — this
   is the highest-financial-risk extraction in the migration plan
   (docs/MIGRATION_PLAN.md §2's RPC/service contract preservation
   checklist), since every atomic invoice/purchase/return RPC call and
   every role-based redaction RPC call flows through here.

   Unlike supabaseClient.js (which builds one client from a hardcoded URL
   and attaches it to `window.SB` as a side effect), this module exports
   a factory so it has no import-time side effects and no dependency on
   `window` — the caller supplies the already-constructed Supabase client
   and the project URL (needed only by parseInvoiceImage, which calls the
   edge function directly via fetch rather than through the client).
   ========================================================================== */
...
export function createSB(supabaseClient: any, supabaseUrl: string) {
  const _sb = supabaseClient;
  ...
  const SB = {
    ...
    async sendEmailOtp(email: string, allowCreate = false) {
    ...
    async sendPhoneOtp(phone: string, allowCreate = false) {
      const { error } = await _sb.auth.signInWithOtp({
        phone,
        options: { shouldCreateUser: allowCreate }
      });
    ...
    async createShopForCurrentUser({ shopName, ownerName, phone, email, address, gstin, stateCode, industry }: any) {
      const { data: { session } } = await _sb.auth.getSession();
    ...
    async updateShopSettings(shopId: string, patch: any) {
      const { error } = await _sb.from('shops').update(patch).eq('id', shopId);
      return { error: error?.message };
    },

    async fetchShopStaff(shopId: string) {
    ...
    async fetchItems(shopId: string) {
    ...
    async upsertCustomer(shopId: string, customer: any) {
      const { data, error } = await _sb
        .from('customers')
    ...
    async fetchCustomers(shopId: string) {
    ...
    async deleteCustomerIfUnused(shopId: string, customerId: string) {
    ...
    async updateCustomerDetails(customerId: string, patch: any) {
    ...
    async fetchVendorDivisions(shopId: string, vendorId: string) {
    ...
    async createVendorDivision(shopId: string, vendorId: string, name: string) {
      const { data, error } = await _sb
    ...
    async setVendorStar(vendorId: string, starred: boolean) {
      const { error } = await _sb.from('vendors').update({ is_starred: starred }).eq('id', vendorId);
      return { error: error?.message };
    },

    async updateVendorDetails(vendorId: string, patch: any) {
    ...
    async saveSale(shopId: string, invoice: any) {
    ...
    async fetchSales(shopId: string, limit = 200) {
    ...
    async fetchProfitSummary(shopId: string, from?: string, to?: string) {
    ...
    async savePurchase(shopId: string, purchase: any) {
    ...
    async fetchVendors(shopId: string) {
      const { data, error } = await _sb
        .from('vendors').select('*').eq('shop_id', shopId).order('name');
      return { data: data || [], error: error?.message };
    },

    async fetchStockAlerts(shopId: string) {
    ...
    async fetchReturnableLines(shopId: string, saleId: string) {
    ...
    async processReturn(shopId: string, saleId: string, ret: any) {
    ...
    async parseInvoiceImage(file: any) {
      const base64: string = await new Promise((resolve, reject) => {
        const reader = new FileReader();
        // readAsDataURL always resolves reader.result to a string.
        reader.onload = () => resolve(/** @type {string} */ (reader.result as string).split(',')[1]);
    ...
    async fetchSubscription(shopId: string) {
    ...
    async adminViewShopSales(shopId: string, reason: string) {
    ...
    async adminSetShopStatus(shopId: string, status: string, reason: string) {
...
export type SBClient = ReturnType<typeof createSB>;
