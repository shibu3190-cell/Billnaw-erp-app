...
- ~~Phase 5: new automated tests required for the atomic RPCs and RLS policies~~ **Done 2026-09-17**, in parallel with Phase 4 rather than waiting for Phase 5 as originally planned — see `tests/rpc-rls-tests.js` (`npm run test:rpc`). Surfaced and led to fixing a real HIGH-severity finding (SECURITY_REPORT.md F0: no INSERT policy on `shops`/`profiles`, confirmed against the live project's actual `pg_policies` to be broken in production, not just a documentation gap). Fixed via `0010_shop_signup_atomic.sql` (a security-definer RPC, not a raw INSERT policy) plus a `supabaseClient.js` update to call it. **Still needs applying to the live Supabase project** — this session has no credentials to do that directly — before Phase 5's service layer can assume shop/profile creation works as documented.
